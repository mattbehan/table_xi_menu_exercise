class MenuCombinationsSolver
	...
	def calculate_remaining_combinations current_iteration
		...
		menu_hash.each do |item, price|
			current_iteration << item
			amount_left = target_price - current_iterations_price(current_iteration)
			if amount_left == 0
			...
			break if @solutions_that_dont_work[convert_iteration_to_key_string(current_iteration)]
